...
export interface ILongword{
    getBit: (iterator: number) => Bit
    setBitAt: (iterator: number, value: Bit) => void
    and: (other: Longword) => Longword
    or: (other: Longword) => Longword
    xor: (other: Longword) => Longword
    rightShift: (amount: number) => Longword
    leftShift: (amount: number) => Longword
